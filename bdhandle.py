...
class BDHandle():
    ...
    def createTable(self):
        """Method for storage table creating """
        try:
            sql_str =f"""CREATE TABLE IF NOT EXISTS {self._tablename}(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT NOT NULL,"""
            for n in self._col_names:
                sql_str+= f'{n} REAL,'
            sql_str =sql_str[:-1] # remove last coma
            sql_str+=');'
            self._lock.acquire()
            ...
            print("Error during table created",e)
